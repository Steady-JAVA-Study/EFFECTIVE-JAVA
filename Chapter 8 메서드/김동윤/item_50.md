...
- 외부 공격으로부터 Period 인스턴스의 내부를 보호하려면 생성자에서 받은 가변 매개변수 각각을 방어적으로 복사(defensive copy) 진행해야 함 
- Period 인스턴스 안에서 원본이 아닌 복사본을 사용
- 매개변수의 유효성 검사하기 전에 방어적 복사본을 만들고, 이 복사본으로 유효성을 검사하는 순서 지키기 
  - 이유 : 원본 객체의 유효성을 검사한 후 복사본을 만드는 그 찰나의 취약한 순간에 다른 스레드가 원본 객체를 수정할 위험이 있기 때문
  - 방어적 복사를 매개변수 유효성 검사 전에 수행하면 이런 위험에서 해방 가능 
- 위와 같이 유효성 검사 순서 변경하고,  접근자가 Period 에 접근하게 두는 것이 아니라, 가변 필드의 방어적 복사본을 반환하도록 적절히 변경해주면  클라이언트가 반환된 Date를 아무리 수정해도 내부의 Date는 변경 x ! 
